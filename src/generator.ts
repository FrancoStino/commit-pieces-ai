...
export async function getSummary(context: vscode.ExtensionContext, diff: string): Promise<string> {
    ...
    const defaultSummaryPrompt = `You are an expert developer specialist in creating commits. Provide a super concise one sentence overall changes summary of the user \`git diff\` output following strictly the next rules: Do not use any code snippets, imports, file routes or bullets points. Do not mention the route of file that has been change. Simply describe the MAIN GOAL of the changes. Output directly the summary in plain text. Translate all in ${language}`.trim();
...
export async function getCommitMessage(context: vscode.ExtensionContext, summaries: string[]): Promise<string> {
    ...
    const defaultCommitPrompt = `You are an expert developer specialist in creating commit messages. Your only goal is to retrieve a single commit message. Based on the provided user changes, combine them in ONE SINGLE commit message retrieving the global idea, following strictly the next rules: - Assign the commit {type} according to the next conditions: feat: Only when adding a new feature. fix: When fixing a bug. docs: When updating documentation. style: When changing elements styles or design and/or making changes to the code style (formatting, missing semicolons, etc.) without changing the code logic. test: When adding or updating tests. chore: When making changes to the build process or auxiliary tools and libraries. revert: When undoing a previous commit. refactor: When restructuring code without changing its external behavior, or is any of the other refactor types. - Do not add any issues numeration, explain your output nor introduce your answer. - Output directly only one commit message of ${commitCharacters} characters in plain text with the next format: \`{type}: {commit_message}\`. - Translate all in ${language}`.trim();
